...
(function (name, definition, global) {
    ...
    var IDBStore = function (kwArgs, onStoreReady) {
        ...
        this.features = { hasAutoIncrement: !env.mozIndexedDB };
        this.consts = {
            'READ_ONLY': 'readonly',
...
define('entity', ['idbstore'], function (IDBStore) {
...
define('../../app/config', [], { storeKey: 'currentKey' });
...
define('../../app/Code', [
    ...
    '../../app/config',
    'Util'
], function (Entity, local, config) {
    var STORE_NAME = 'Code', TITLE_LENGTH = 10;
...
(function (mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        ...
        define('javascript', ['codemirror'], mod);
    else
        mod(CodeMirror);
}(function (CodeMirror) {
    'use strict';
...
(function (mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        mod(require('codemirror'), require('../xml/xml'), require('../javascript/javascript'), require('../css/css'));
    else if (typeof define == 'function' && define.amd)
        define('htmlmixed', [
            'codemirror',
            'xml',
    ...
    function findMatchingMode(tagInfo, stream) {
        for (var i = 0; i < tagInfo.length; i++) {
            var spec = tagInfo[i];
            if (!spec[0] || spec[1].test(getAttrValue(stream, spec[0])))
                return spec[2];
        }
    }
    CodeMirror.defineMode('htmlmixed', function (config, parserConfig) {
        ...
        function html(stream, state) {
            ...
            if (tagInfo && /\btag\b/.test(style) && stream.current() === '>' && (modeSpec = findMatchingMode(tagInfo, stream))) {
...
(function (mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        ...
        define('closebrackets', ['codemirror'], mod);
    else
        mod(CodeMirror);
}(function (CodeMirror) {
    var defaults = {
...
(function (mod) {
    if (typeof exports == 'object' && typeof module == 'object')
        ...
        define('xml-fold', ['codemirror'], mod);
    else
        mod(CodeMirror);
}(function (CodeMirror) {
    'use strict';
...
define('../../app/editor', [
    ...
    var html_template = '<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"/><title>Demo</title><style>{1}</style></head><body>{0}<script>{2}</script></body></html>', html_e = document.getElementById('html'), css_e = document.getElementById('css'), js_e = document.getElementById('javascript'), STATUS_KEY = 'isPlay';
    ...
    function play(mustRun) {
        if (!mustRun && local(STATUS_KEY) === 'true')
            return;
        var html = this.html.getValue(), css = this.css.getValue(), js = this.js.getValue(), resultDoc = window.frames[0].document;
        resultDoc.open();
        resultDoc.write(html_template.format(html, css, js));
        resultDoc.close();
    }
    function styleToggle(isInit) {
        var editorsStyle = document.getElementById('editors').style, resultStyle = document.getElementById('result').style, playEle = document.querySelector('[data-action="play"]'), isReversal = local(STATUS_KEY) === 'true';
...
require([
    ...
    require([
        ...
        new keyMap().keyBind([
            ...
            editor.format(editor.html).format(editor.css).format(editor.js);
        });
        require([
            'emmet',
            'show-hint',
